...
export default class BelaData extends BelaWebSocket {
	constructor(port=5555, address='gui_data', ip='192.168.7.2') {
	...
	onData (data) {
		...
		console.log("[BelaData] onData():", data)
	    ...
	    if(this.currentState == this.states[0]) { // buffer id
            ...
            switch(this.newBuffer['type']) {
                ...
                case 'd':
                    ...
                    break;
                default:
                    console.log("[BelaData] onData(): Unknown buffer type");
            }
	...
	sendBuffer (id, type, data) {
	    ...
	    if      (type == 'float' || type == 'f') type = 'f'
		else if (type == 'int'   || type == 'd') type = 'd'
	    else if (type == 'char'  || type == 'c') type = 'c'
		else return false
	...
	formatPkt (id, type, dataArray) {
	    ...
	    if      (type === 'd' || type === 'f') pktLen += Float32Array.BYTES_PER_ELEMENT*arrayLen
	    else if (type === 'c') pktLen += arrayLen;
		else return null
	    ...
	    let arrayView;
	    if      (type === 'd') arrayView = new Int32Array   (buffer, header.size, arrayLen);
	    else if (type === 'f') arrayView = new Float32Array (buffer, header.size, arrayLen);
	    else if (type === 'c') arrayView = new Uint8Array   (buffer, header.size, arrayLen);
	    else return null

	    for(let i=0; i<arrayLen; i++) {
	        if(type === 'c') arrayView[i] = dataArray[i].charCodeAt(0);
	        else arrayView[i] = dataArray[i];
	    }
	    return buffer
	}
